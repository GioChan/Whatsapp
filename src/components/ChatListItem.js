...
class ChatListItem extends React.Component<Props, State> {
  ...
  render() {
    const { id, title, preview, tanggal, avatar, showChatScreen } = this.props;
    return (
      <StyledListItem>
        <useTransition
        ...
        >
          {(show) =>
            show &&
            ((props) => (
              <ContactDetailOverlay title={title} avatar={avatar} style={props} toggleAvatar={this.toggleAvatar} />
            ))
          }
        </useTransition>
        <ListItemAvatar onClick={this.toggleAvatar} avatarIsOpen={this.state.avatarIsOpen}>
